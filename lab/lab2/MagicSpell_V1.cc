...
bool evalPos(short x,short y,char v,vector<groupTerms> *gta){
    if(v == 'x'){
        double lsum = 0,ysum=0;//lsum(literal sum) ysum (for n*y terms)
        // short cY1=0,cY2=0;
        ...
        // y(y+n) = lsum
        // cout << ysum << " " << lsum << endl;
        if(ysum == lsum)return 1;

    }else if (v == 'y'){
        // short cY1=0,cY2=0;
        double lsum = 0,xsum=0;//lsum(literal sum) xsum (for n*x terms)
        for(short i = 0;i<gta->size();i++){
            string type = gta->at(i).type;
            double n = gta->at(i).n;
            if(type.find("y") != string::npos){
                if(type.find("y^2") != string::npos){
                    lsum += n * pow(x,2);
                }else{
                    lsum += n * x;
                }
            }else if (type.find("x") != string::npos){
                // cout << n << type << "| ";
                if(type.find("x^2") != string::npos){
                    // cY2++;
                    xsum += n * pow(y,2);
